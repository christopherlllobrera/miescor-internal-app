...
class ActivityLog extends SpatieActivity
{
    ...
    public function getPropertiesForDisplayAttribute(): string
    {
        if (empty($this->properties)) {
            return '—';
        }

        // Pretty-print as readable JSON
        return json_encode($this->properties, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    }

    public function getFlatPropertiesAttribute(): array
    {
        $properties = $this->properties ?? [];
        $flat = [];
        ...
        foreach ($properties as $key => $value) {
            if (is_array($value)) {
                foreach ($value as $subKey => $subValue) {
                    $flat["{$key}.{$subKey}"] = is_array($subValue)
